...
passport.use(new LocalStrategy({ usernameField: 'email' }, (email, password, done) => {
  User.findOne({ email: email.toLowerCase() }, (err, user) => {
    ...
    if (!user) { return done(null, false, 'Credenciales Invalidas.'); }
...
function signup({ email, password, req }) {
  ...
  if (!email || !password) { throw new Error('Por favor ingresa un email y un password.'); }

  return User.findOne({ email })
    .then(existingUser => {
      if (existingUser) { throw new Error('Email en uso'); }
      return user.save();
    })
    .then(user => {
...
function login({ email, password, req }) {
  return new Promise((resolve, reject) => {
    passport.authenticate('local', (err, user) => {
      if (!user) { reject('Credenciales Invalidas.') }
